...
use PgFunc\Exception\InvalidIsolationLevelException;
...
class Connection
{
    ...
    {
        [$sql, $parameters] = $procedure->generateQueryData();
     ...
     */
    final public function createTransaction() : Transaction
    {
     ...
     * @return self
     */
    final public function setIsTransactionEnabled($isTransactionEnabled) : self
    {
     ...
     */
    private function applyLocalParams(& $sql, array & $parameters, array $localParams) : void
    {
        $sqlParts = [];
     ...
     */
    private function getStatement($sql) : PDOStatement
    {
        try {
     ...
     *
     */
    private function getFlags($value) : int
    {
        switch (true) {
            case null === $value:
     ...
     */
    private function executeStatement(PDOStatement $statement, Procedure $procedure) : ?Database
    {
        try {
            ...
            [$defaultIsolationLevel] = $this->setIsolationLevel(static::SERIALIZABLE);
            $statement->execute();
            $this->setIsolationLevel($defaultIsolationLevel);
     ...
     */
    private function prepareIsJsonAsArray(Procedure $procedure) : bool
    {
        $isJsonAsArray = $procedure->isJsonAsArray();
        if (null === $isJsonAsArray) {
     ...
     * @throws Specified When database exception is known and specified.
     */
    private function handleException(PDOException $exception, Procedure $procedure) : Database
    {
     ...
     * @return int
     */
    public function setSearchPath(string $searchPath) : int
    {
        return $this->db->exec(sprintf('SET search_path TO %s;', $searchPath));
    }
}
